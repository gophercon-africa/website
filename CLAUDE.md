...
- `npm run build` — runs `prisma generate` then `next build` (webpack). Uses a separate build ESLint config.
- `npm run lint`, and `npx tsc --noEmit` for type-checking.
